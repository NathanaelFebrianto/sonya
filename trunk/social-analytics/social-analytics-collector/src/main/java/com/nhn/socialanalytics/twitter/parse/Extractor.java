...
public class Extractor {
	...
	public List<String> extractMentionedScreennames(String text) {
		...
		while (matcher.find()) {
			if (!Regex.SCREEN_NAME_MATCH_END.matcher(
					matcher.group(Regex.EXTRACT_MENTIONS_GROUP_AFTER))
					.matches()) {
				extracted.add(matcher
	...
	public List<String> extractHashtags(String text) {
		...
		return extractList(Regex.AUTO_LINK_HASHTAGS, text,
	...
	public List<Entity> extractHashtagsWithIndices(String text) {
		...
		return extractListWithIndices(Regex.AUTO_LINK_HASHTAGS, text,
				Regex.AUTO_LINK_HASHTAGS_GROUP_TAG, "hashtag");
	}
	 ...
	 */
	private List<String> extractList(Pattern pattern, String text,
			int groupNumber) {
		List<String> extracted = new ArrayList<String>();
	...
	private List<Entity> extractListWithIndices(Pattern pattern, String text,
