...
colnames(df_audiences_by_tweet_type) <- c("user", "tweet_type", "create_date", "at.total_tweet_count", "at.total_user_count", 
		"at.positive_tweet_count", "at.positive_user_count", "at.negative_tweet_count", "at.negative_user_count")
...
#	tweet.types <- c("REPLY", "MENTION")	
	...
	cor_pos = cor(df_user$at.positive_tweet_count, df_user$p.positive_tweet_count)	
	cor_neg = cor(df_user$at.negative_tweet_count, df_user$p.negative_tweet_count)
	cat(user.name, "\n")
