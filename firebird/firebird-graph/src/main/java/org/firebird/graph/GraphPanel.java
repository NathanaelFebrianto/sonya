...
public class GraphPanel extends JPanel {
	...
	public GraphPanel() {				
		...
		add(createMenu(toolbar), BorderLayout.NORTH);
		add(panelViewer, BorderLayout.CENTER);
	}
	
	private JTabbedPane createMenu (GraphToolBar toolbar) {
		JTabbedPane menu = new JTabbedPane();
		
		List<JPanel> toolbarList = toolbar.getToolBarList();
		for (int i = 0; i < toolbarList.size(); i++) {
			JPanel panel = (JPanel)toolbarList.get(i);
			menu.addTab(panel.getToolTipText(), panel);
		}		
		return menu;
	}
